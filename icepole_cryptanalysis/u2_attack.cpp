...
int attack_u2(const char * logcat, const u_int8_t * key, const u_int8_t * iv, u_int64_t & U2, const u_int64_t & U0, const u_int64_t & U3, size_t & generated_p2s)
{
	int result = -1;
	...
	log4cpp::Category::getInstance(logcat).debug("%s: provided: U0 = 0x%016lX; U3 = 0x%016lX;", __FUNCTION__, U0, U3);
					...
					if(NULL != timer_evt)
					{
						...
						if(0 == event_add(timer_evt, &_3sec))
						{
							...
							for(size_t i = 0; i < thread_count; ++i)
							{
								...
								atckr_prms[i].required_attacks = (pow(2, 31.2)/thread_count)+1;//(pow(2, 31.7)/thread_count)+1;
