					...
					if(NULL != timer_evt)
					{
						...
						if(0 == event_add(timer_evt, &_3sec))
						{
							...
							for(size_t i = 0; i < thread_count; ++i)
							{
								...
								atckr_prms[i].required_attacks = (pow(2, 31.2)/thread_count)+1;//(pow(2, 32.4)/thread_count)+1;
			   ...
			   const u_int64_t init_state[4][5], aes_prg & prg, bit_ctrs_t ctrs[64], u_int64_t & generated_p2s)
{
	...
	{
		u_int8_t F1, F2;
		if(last_Sbox_lookup_filter((C1+BLONG_SIZE), bit, u1_omega_bits, 6, F1))
		{
			...
			if(last_Sbox_lookup_filter((C2+BLONG_SIZE), bit, u1_omega_bits, 6, F2))
			{
				ctrs[bit].ctr_1[0]++;
				if(F1 == F2)
					ctrs[bit].ctr_2[0]++;
